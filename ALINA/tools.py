...
from ALINA.info import (get_served_chats, get_served_users, del_served_chat, del_served_user, activecall, add_active_chat, add_served_call, add_active_video_chat)
from ALINA.Data import Bots
from ALINA.play import (logs, join_call)
from ALINA.Data import (get_userbot, get_dev, get_call, get_group, get_channel)
import aiohttp
import asyncio
    ...
    for served_chat in served_chats:
        if f"{served_chat}" in text:
         ...
         try:
            ...
            count += 1
            txt = f"{count}:- گرووپ : [{title}](https://t.me/{username}) ئایدی : `{served_chat}`\n" if username else f"{count}:- گرووپ : {title} ئایدی : `{served_chat}`\n"
            text += txt
         except Exception:
            title = "Not Found" 
            count += 1
...
@Client.on_message(filters.command(["• بەشی فۆروارد •", "• گەڕانەوە •", "بەشی فۆروارد"], ""))
async def cast(client: Client, message):
   bot_username = client.me.username
   dev = await get_dev(bot_username)
   if message.chat.id == dev or message.chat.username in OWNER:
    kep = ReplyKeyboardMarkup([["• فۆرواردی گشتی •"], ["• فۆروارد بۆ گرووپەکان •", "• فۆروارد بۆ بەکارهێنەرەکان •"], ["• ڕێکڵامی گشتی •"], ["• ڕێکڵام بۆ گرووپەکان •", "• ڕێکڵام بۆ بەکارهێنەرەکان •"], ["• گەڕانەوە بۆ لیستی سەرەکی •"]], resize_keyboard=True)
    await message.reply_text("**👋🏻 ꒐ بەخێربێی گەشەپێدەری ئەزیز **\n**⚡ ꒐ بۆ بەشی فۆروارد**", reply_markup=kep)


@Client.on_message(filters.command(["• فۆرواردی گشتی •", "• فۆروارد بۆ گرووپەکان •", "• فۆروارد بۆ بەکارهێنەرەکان •", "• ڕێکڵامی گشتی •", "• ڕێکڵام بۆ بەکارهێنەرەکان •", "• ڕێکڵام بۆ گرووپەکان •"], ""))
async def cast1(client: Client, message):
   ...
   if message.chat.id == dev or message.chat.username in OWNER:
    if command == "• فۆرواردی گشتی •":
     kep = ReplyKeyboardMarkup([["• فۆرواردی گشتی بە بۆت •"], ["• فۆرواردی گشتی بە یاریدەدەر •"], ["• گەڕانەوە •"]], resize_keyboard=True)
     await message.reply_text("**👋🏻 ꒐ بەخێربێی گەشەپێدەری ئەزیز **\n**⚡ ꒐ بۆ بەشی فۆروارد**", reply_markup=kep)
    elif command == "• فۆروارد بۆ گرووپەکان •":
     kep = ReplyKeyboardMarkup([["• فۆروارد بۆ گرووپەکان بە بۆت •"], ["• فۆروارد بۆ گرووپەکان بە یاریدەدەر •"], ["• گەڕانەوە •"]], resize_keyboard=True)
     await message.reply_text("**👋🏻 ꒐ بەخێربێی گەشەپێدەری ئەزیز **\n**⚡ ꒐ بۆ بەشی فۆروارد**", reply_markup=kep)
    elif command == "• فۆروارد بۆ بەکارهێنەرەکان •":
     kep = ReplyKeyboardMarkup([["• فۆروارد بۆ بەکارهێنەرەکان بە بۆت •"], ["• فۆروارد بۆ بەکارهێنەرەکان بە یاریدەدەر •"], ["• گەڕانەوە •"]], resize_keyboard=True)
     await message.reply_text("**👋🏻 ꒐ بەخێربێی گەشەپێدەری ئەزیز **\n**⚡ ꒐ بۆ بەشی فۆروارد**", reply_markup=kep)
    elif command == "• ڕێکڵامی گشتی •":
     kep = ReplyKeyboardMarkup([["• ڕێکڵامی گشتی بە بۆت •"], ["• گەڕانەوە •"]], resize_keyboard=True)
     await message.reply_text("**👋🏻 ꒐ بەخێربێی گەشەپێدەری ئەزیز **\n**⚡ ꒐ بۆ بەشی فۆروارد**", reply_markup=kep)
    elif command == "• ڕێکڵام بۆ بەکارهێنەرەکان •":
     kep = ReplyKeyboardMarkup([["• ڕێکڵام بۆ بەکارهێنەرەکان بە بۆت •"], ["• گەڕانەوە •"]], resize_keyboard=True)
     await message.reply_text("**👋🏻 ꒐ بەخێربێی گەشەپێدەری ئەزیز **\n**⚡ ꒐ بۆ بەشی فۆروارد**", reply_markup=kep)
    else:
     kep = ReplyKeyboardMarkup([["• ڕێکڵام بۆ گرووپەکان بە بۆت •"], ["• گەڕانەوە •"]], resize_keyboard=True)
     await message.reply_text("**👋🏻 ꒐ بەخێربێی گەشەپێدەری ئەزیز **\n**⚡ ꒐ بۆ بەشی فۆروارد**", reply_markup=kep)


@Client.on_message(filters.command(["• فۆرواردی گشتی بە یاریدەدەر •", "• فۆرواردی گشتی بە بۆت •", "• فۆروارد بۆ گرووپەکان بە یاریدەدەر •", "• فۆروارد بۆ گرووپەکان بە بۆت •", "• فۆروارد بۆ بەکارهێنەرەکان بە یاریدەدەر •", "• فۆروارد بۆ بەکارهێنەرەکان بە بۆت •", "• ڕێکڵامی گشتی بە یاریدەدەر •" ,"• ڕێکڵامی گشتی بە بۆت •" ,"• ڕێکڵام بۆ گرووپەکان بە یاریدەدەر •" ,"• ڕێکڵام بۆ گرووپەکان بە بۆت •" ,"• ڕێکڵام بۆ بەکارهێنەرەکان بە یاریدەدەر •" ,"• ڕێکڵام بۆ بەکارهێنەرەکان بە بۆت •"], ""))
async def cast5(client: Client, message):
  command = message.command[0]
  bot_username = client.me.username
  dev = await get_dev(bot_username)
  if message.chat.id == dev or message.chat.username in OWNER:
   kep = ReplyKeyboardMarkup([["• هەڵوەشاندنەوە •"], ["• گەڕانەوە •"], ["• گەڕانەوە بۆ لیستی سەرەکی •"]], resize_keyboard=True)
   ask = await client.ask(message.chat.id, "**ئێستا ئەو شتە بنێرە کە دەتەوێت بینێرم ⚡.**", reply_markup=kep)
   x = ask.id
   y = message.chat.id
                ...
                try:
                 await m.pin(disable_notification=False)
                   ...
                   continue
                    ...
                    fd += 1
                    continue
     return await message.reply_text(f"**بە سەرکەوتوویی فۆروارد کرا ⚡**\n\n**بۆ : {dn}**\n**شکستی هێنا لە : {fd}**")
...
@Client.on_message(filters.command(["• بەشی ئەکاونتی یاریدەدەر •", "بەشی ئەکاونتی یاریدەدەر"], ""))
async def helpercn(client, message):
   ...
   if message.chat.id == dev or message.chat.username in OWNER:
    kep = ReplyKeyboardMarkup([["• پشکنینی یاریدەدەر •"], ["• گۆڕینی ناوی یەکەم •", "• گۆڕینی ناوی دووەم •"], ["• گۆڕینی بایۆ •"], ["• گۆڕینی یوزەری یاریدەدەر •"], ["• گۆڕینی وێنە •", "• لادانی وێنە •"], ["• بانگکردنی یاریدەدەر •"], ["• گەڕانەوە بۆ لیستی سەرەکی •"]], resize_keyboard=True)
    await message.reply_text(f"**👋🏻 ꒐ بەخێربێی گەشەپێدەری ئەزیز **\n**⚡ ꒐ بۆ بەشی ئەکاونتی یاریدەدەر**\n**{me.mention}**\n**{i}**\n**{b}**", reply_markup=kep)
...
async def changefisrt(client: Client, message):
  ...
  if message.chat.id == dev or message.chat.username in OWNER:
   try:
    ...
    await message.reply_text("**بە سەرکەوتوویی ناوی ئەکاونتی یاریدەدەر گۆڕا ⚡.**")
...
@Client.on_message(filters.command(["• گۆڕینی یوزەری یاریدەدەر •", "گۆڕینی یوزەری یاریدەدەر"], ""))
async def changeusername(client: Client, message):
  bot_username = client.me.username
  dev = await get_dev(bot_username)
  if message.chat.id == dev or message.chat.username in OWNER:
   try:
    name = await client.ask(message.chat.id, "**• ئێستا یوزەرە نوێیەکە بنێرە •**")
    name = name.text
    client = await get_userbot(bot_username)
    await client.set_username(name)
    await message.reply_text("**بە سەرکەوتوویی یوزەری یاریدەدەر گۆڕا ⚡.**")
   except Exception as es:
     await message.reply_text(f"**• هەڵە ڕوویدا لە گۆڕینی یوزەری یاریدەدەر\n {es} **")
...
async def changephoto(client: Client, message):
  ...
  if message.chat.id == dev or message.chat.username in OWNER:
   try:
    ...
    await message.reply_text("***بە سەرکەوتوویی وێنەی ئەکاونتی یاریدەدەر گۆڕا ⚡.**") 
   except Exception as es:
...
async def joined(client: Client, message):
  ...
  if message.chat.id == dev or message.chat.username in OWNER:
   try:
    ...
    await message.reply_text("**بە سەرکەوتوویی ئەکاونی یاریدەدەر جۆینی کرد ⚡**")
   except Exception as es:
     await message.reply_text(f"** هەڵە ڕوویدا لە کاتی جۆین کردن \n {es}**")



@Client.on_message(filters.command(["• گۆڕینی گرووپی ئامار •", "• چالاککردنی گرووپی ئامار •", "• ناچالاککردنی گرووپی ئامار •"], ""))
async def set_history(client: Client, message):
 bot_username = client.me.username
 dev = await get_dev(bot_username)
 if message.chat.id == dev or message.chat.username in OWNER:
  if message.command[0] == "• گۆڕینی گرووپی ئامار •":
   ask = await client.ask(message.chat.id, "** یوزەر یان ئایدی گرووپ بنێرە **", timeout=30)
   logger = ask.text
   if "@" in logger:
        ...
        if message.command[0] == "• گۆڕینی گرووپی ئامار •":
         if i["logger"] == logger:
          ...
          try:
           ...
           await user.send_message(logger, "**گرووپی ئامار دەگۆڕم ..**")
           ...
           await ask.reply_text("**بە سەرکەوتوویی گرووپی ئامار گۆڕا ✅**")
          except Exception:
            await ask.reply_text("**دڵنیابە کە بۆت و ئەکاونی یاریدەدەرت زیادکردووە و کردووتە بە ئەدمین**")
        else:
         mode = "ON" if message.command[0] == "• چالاککردنی گرووپی ئامار •" else "OFF"
         if i["logger_mode"] == mode:
           m = "چالاککراوە" if message.command[0] == "• چالاککردنی گرووپی ئامار •" else "ناچالاککراوە"
           return await message.reply_text(f"**گرووپی ئامار پێشتر {m} ⚡.**")
         else:
          try:
           ...
           m = "چالاککرا" if message.command == "• چالاککردنی گرووپی ئامار •" else "ناچالاککرا"
           await message.reply_text(f"**بە سەرکەوتوویی گرووپی ئامار {m}✅**")
          except Exception as es:
            await message.reply_text("**هەڵە ڕوویدا پەیوەندی بە گەشەپێدەر بکە ..**")
