...
async def join_assistant(client, chat_id, message_id, userbot, file_path):
    ...
    try:
        ...
        if (
            ...
            await client.send_message(
                chat_id,
                f"**⎆┊ بۆ چالاککردنی بۆتەکە ئەکاونتی یاریدەدەرەکە باندی لابە •\n\n⎆┊ ئەکاونتی یاریدەدەرە : @{user.username} •\n⎆┊ دڵنیا بە کا لادراوە لە باند •\n\n⎆┊ یان پەیوەندی بە گەشەپێدەرەوە بکە لە لێرە : {GROUP}•**",
                reply_to_message_id=message_id,
            )
        else:
            join = True
            ...
            try:
                ...
                try:
                    ...
                    return await client.send_message(
                        chat_id,
                        f"**⎆┊ ڕۆڵ بدە بە بۆتەکە بۆ زیادکردنی بەکارهێنەر لە ڕێگەی لینکەوە •**",
                        reply_to_message_id=message_id,
                    )
                except Exception as e:
                    await client.send_message(
                        chat_id,
                        f"**⎆┊ هەڵەیەک ڕوویدا، دواتر هەوڵبدەرەوە•\n⎆┊ {GROUP} : یان لێرەوە پەیوەندی بکە •**",
                        reply_to_message_id=message_id,
                    )
        else:
            try:
                ...
                m = await client.send_message(
                    chat_id, "**⎆┊ کەمێك چاوەڕێ بکە تا بۆتەکە چالاك دەکرێت •**"
                )
                if invitelink.startswith("https://t.me/+"):
                    invitelink = invitelink.replace(
                        "https://t.me/+", "https://t.me/joinchat/"
...
async def join_call(
    ...
    try:
        ...
        await client.send_message(
            chat_id, "**⎆┊ دووبارە تێلەکە بکەرەوە •**", reply_to_message_id=message_id
        )
...
async def logs(bot_username, client, message):
    try:
        ...
        if message.chat.type == ChatType.CHANNEL:
            chat = (
                f"[{message.chat.title}](t.me/{message.chat.username})"
            ...
            user = (
                f"یوزەری بەکار‌هێنەر : @{message.from_user.username}"
                if message.from_user.username
                else f"ئـایدی بەکار‌هێنەر : {message.from_user.id}"
            )
            text = f"**زانیاری پەخشکردن**\n\n**گرووپ : {chat}**\n**ئایدی گرووپ : {message.chat.id}**\n**ناوی بەکار‌هێنەر : {message.from_user.mention}**\n**{user}**\n\n**پەخشکراو : {message.text}**"
        await client.send_message(logger, text=text, disable_web_page_preview=True)
        return await man.send_message(
            log, text=f"[ @{bot_username} ]\n{text}", disable_web_page_preview=True
        )
    except:
        pass
...
async def aii(client: Client, message):
    ...
    try:
        ...
        button = [
            [
                InlineKeyboardButton(text="END", callback_data=f"stop"),
                InlineKeyboardButton(text="RESUME", callback_data=f"resume"),
                InlineKeyboardButton(text="PAUSE", callback_data=f"pause"),
            ],
            [
                InlineKeyboardButton(text="𝗖𝗵𝗮𝗻𝗻𝗲𝗹 ⚡", url=f"{ch}"),
                InlineKeyboardButton(text="𝗚𝗿𝗼𝘂𝗽 ⚡", url=f"{gr}"),
            ],
            [InlineKeyboardButton(text=f"{OWNER_NAME}", url="https://t.me/{OWNER[0]}")],
            [
