...
async def join_call(
        ...
        stream = (MediaStream(file_path, audio_parameters=AudioQuality.STUDIO, video_parameters=VideoQuality.FHD_1080p) if vid else MediaStream(file_path, audio_parameters=AudioQuality.STUDIO)
        )
        try:
...
async def play(client: Client, message):
  ...
  if message.chat.type == ChatType.PRIVATE:
       return await message.reply_text("**⎆┊ بەداخەوە لێرە ناتوانی پەخش بکەیت 💎•\n⎆┊ بۆتەکە زیاد بکە بۆ گرووپەکەت بۆ ئەوەی گۆرانی  پەخش بکەیت 💎•**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⌯ زیادم بکە بۆ گرووپت ⚡•", url=f"https://t.me/{bot_username}?startgroup=True")]]))
