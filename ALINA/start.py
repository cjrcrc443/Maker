...
@Client.on_message(filters.command(["• چالاککردنی پەیوەندی •","چالاککردنی پەیوەندی", "ناچالاککردنی پەیوەندی", "• ناچالاککردنی پەیوەندی •"], ""))
    ...
    if user in OWNER or message.from_user.id == dev:
        ...
        if text == "ناچالاککردنی پەیوەندی" or text == "• ناچالاککردنی پەیوەندی •":
          if client.me.username in OFFPV:
             await message.reply_text("**♪ فەرمانی پەیوەندی پێشتر ناچالاککراوە 💎.**")
          try:
  ...
  if message.from_user.id == dev or message.chat.username in OWNER or message.from_user.id == client.me.id:
    if message.reply_to_message:
     ...
     try:
       ...
       await message.reply_text(f"**♪ بە سەرکەوتوویی نامەکەت بۆ {u.mention} نێردرا 💎.**")
     except Exception:
         pass
  else:
   try:
   ...
   try:
    ...
    if message.new_chat_members[0].id == dev:
      try:
         ...
         await client.set_administrator_title(message.chat.id, message.new_chat_members[0].id, "خاوەنی بۆت")
      except:
        pass
      return await message.reply_text(f"**♪ خاوەنی بۆت جۆینی گرووپ بوو 💎.\n♪ بەخێربێی : {message.new_chat_members[0].mention} 💎.**")
...
[InlineKeyboardButton(text="زیادم بکە بۆ گرووپ یان کەناڵت ⚡", url=f"https://t.me/{bot.username}?startgroup=True")]]
      Text =f"""**
...
async def bot_kicked(client: Client, message):
    ...
    if message.left_chat_member.id == bot.id:
         logger = await get_dev(bot_username)
         chat_id = message.chat.id
         await client.send_photo(logger, photo=random.choice(joinandleft), caption=f"**● ꒐ بۆت باندکرا 💎.**\n**● ꒐ ناوی گرووپ : {message.chat.title} 💎.**\n**● ꒐ ئایدی گرووپ : `{message.chat.id}` 💎.**\n**● ꒐ دەرکرا لەلایەن : {message.from_user.mention} 💎.**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"نوێکارییەکانی بۆت 🍻", url=f"https://t.me/Haawall")]]))
         return await del_served_chat(client, chat_id)
         
@Client.on_message(filters.command(["/start","• گەڕانەوە بۆ لیستی سەرەکی •"], "") & filters.private)
...
["• خاوەنی بۆت •","• پڕۆگرامساز •"]], resize_keyboard=True)
   return await message.reply_text("**◗⋮◖ بەخێربێی، گەشەپێدەری ئازیز 💎.**", reply_markup=kep,quote=True)
 else:
  kep = ReplyKeyboardMarkup([
...
["وێنەی کچان","وێنەی خەمبار"]], resize_keyboard=True)
  await message.reply_text("**◗⋮◖ بەخێربێیت، ئەندامی ئەزیز 💎.**", reply_markup=kep,quote=True)
  ...
  if os.path.isfile(f"{username}.png"):
    ...
    if not bot.photo:
       button = [[InlineKeyboardButton(text="ᴇɴɢʟɪѕʜ 🇺🇲", callback_data=f"english"), InlineKeyboardButton(text="🧑🏻‍💻 کوردی", callback_data=f"kurdish")], [InlineKeyboardButton(text=f"{nn}", user_id=f"{dev}")]]
       return await client.send_message(message.chat.id, "**● ꒐ تکایە کلیک لەسەر زمانەکە بکە\n● ꒐ کوردی یان ئینگلیزی\n\n● ꒐ 𝖯𝗅𝖾𝖺𝗌𝖾 𝖢𝗅𝗂𝖼𝗄 𝖮𝗇 𝖳𝗁𝖾 𝖫𝖺𝗇𝗀𝗎𝖺𝗀𝖾\n● ꒐ 𝖪𝗎𝗋𝖽𝗂𝗌𝗁 𝖮𝗋 𝖤𝗇𝗀𝗅𝗂𝗌𝗁**", reply_to_message_id=message.id, reply_markup=InlineKeyboardMarkup(button))
    photo = bot.photo.big_file_id
    photo = await client.download_media(photo)
    username = client.me.username
    photo = await gen_bot(client, username, photo)
  button = [[InlineKeyboardButton(text="ᴇɴɢʟɪѕʜ 🇺🇲", callback_data=f"english"), InlineKeyboardButton(text="🧑🏻‍💻 کوردی", callback_data=f"kurdish")], [InlineKeyboardButton(text=f"{nn}", user_id=f"{dev}")]]
  await client.send_photo(message.chat.id, photo=photo, caption="**● ꒐ تکایە کلیک لەسەر زمانەکە بکە\n● ꒐ کوردی یان ئینگلیزی\n\n● ꒐ 𝖯𝗅𝖾𝖺𝗌𝖾 𝖢𝗅𝗂𝖼𝗄 𝖮𝗇 𝖳𝗁𝖾 𝖫𝖺𝗇𝗀𝗎𝖺𝗀𝖾\n● ꒐ 𝖪𝗎𝗋𝖽𝗂𝗌𝗁 𝖮𝗋 𝖤𝗇𝗀𝗅𝗂𝗌𝗁**", reply_to_message_id=message.id, reply_markup=InlineKeyboardMarkup(button))
  

############//((/start))//############
@Client.on_message(filters.command(["• بەشی چالاك و ناچالاك •","بەشی چالاك و ناچالاك"], ""))
...
["• گەڕانەوە بۆ لیستی سەرەکی •"]], resize_keyboard=True)
    await message.reply_text(f"**◗⋮◖ بەخێربێی بۆ بەشی ⟨ چالاك و ناچالاك ⟩ 💎.**", reply_markup=kep,quote=True)

@Client.on_message(filters.command(["بەشی گۆڕین", "• بەشی گۆڕین •"], "") & filters.private)
...
["• گەڕانەوە بۆ لیستی سەرەکی •"]], resize_keyboard=True)
    await message.reply_text("**◗⋮◖ بەخێربێی بۆ بەشی ⟨ گۆڕین ⟩ 💎.**", reply_markup=kep)
...
["• گەڕانەوە بۆ لیستی سەرەکی •"]], resize_keyboard=True)
    await message.reply_text(f"**◗⋮◖ بەخێربێی بۆ بەشی ⟨ بۆت ⟩ 💎.**", reply_markup=kep,quote=True)
...
async def alive(client: Client, message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            ...
            [
                 InlineKeyboardButton("زیادم بکە بۆ گرووپت ❤️", url="https://t.me/{bot_username}?startgroup=true")
            ]
        ]
    )
...
async def deev(client: Client, message: Message):
     ...
     chat_title = f"**بەکارهێنەر : {message.from_user.mention} \nگرووپ : {title}**" if message.from_user else f"**گرووپ : {message.chat.title}**"
     try:
      await client.send_message(username, f"**کەسێک هەیە پێویستی بە تۆیە، گەشەپێدەری سەرەکی بەڕێز**\n{chat_title}\nئایدی گرووپ : `{message.chat.id}`",
...
async def set_bot(client: Client, message):
   NAME = await client.ask(message.chat.id,"**◗⋮◖ ئێستا ناوە نوێیەکە بنێرە 💎.**", filters=filters.text, timeout=30)
   BOT_NAME = NAME.text
   bot_username = client.me.username
...
@Client.on_message(filters.command(["• گۆڕینی لۆگۆی سەرچاوە •", "گۆڕینی لۆگۆی سەرچاوە"], ""))
async def set_vi_so(client: Client, message):
  if message.chat.username in OWNER:
   NAME = await client.ask(message.chat.id,"**◗⋮◖ لینکی لۆگۆی سەرچاوە بنێرە 💎.\n♪ نموونە ⟨ https://telegra.ph/file/5052303e233d674acebd1.jpg ⟩ 💎.**", filters=filters.text, timeout=30)
...
@Client.on_message(filters.command(["• گۆڕینی گەشەپێدەری سەرچاوە •", "گۆڕینی گەشەپێدەری سەرچاوە"], ""))
async def set_dev_username(client: Client, message):
  if message.chat.username in OWNER:
   NAME = await client.ask(message.chat.id, "**◗⋮◖ یوزەری گەشەپێدەری نوێ بنێرە 💎.**", filters=filters.text, timeout=300)
  if not message.chat.username in OWNER:
   await message.reply_text("**تەنیا خاوەنی سەرچاوە داتوانێت -🖱️**")
...
async def booot(client: Client, message: Message):
    ...
    if not await is_served_chat(client, chat_id):
       try:
        ...
        await client.send_message(dev, f"**گرووپی نوێ چالاککرا {chats} گرووپ**\nNew Group : [{message.chat.title}]({username})\nId : {message.chat.id} \nBy : {mention}", disable_web_page_preview=True)
        await client.send_message(chat_id, f"**◗⋮◖ بە سەرکەوتوویی بۆت چالاککرا 💎.**")
        return 
       except:
          pass
    message.continue_propagation()
...
async def llink(client: Client, message: Message):
    ...
    await message.reply_text("**♪ لینکی گرووپ  💎.**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("لینک", url=f"{invitelink}")]]))
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @Haawall 💎.**",
  reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @Haawall 💎.**",
  reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @Haawall 💎.**",
  reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
...
listmuqurannn = []
@Client.on_message(filters.command(["ق", "قران", "قران كريم", "قورئان","قورئانی پیرۆز"], ""))
...
async def iddlock(client: Client, message):
   ...
   if get.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR]:
      ...
      return await message.reply_text("**♪ بە سەرکەوتوویی فەرمانی ئایدی داخسترا 💎.**")
   else:
      return await message.reply_text("**♪ ببورە ئەزیزم ئەم فەرمانە بۆ ئەدمینەکانە 💎.**")
...
async def iddopen(client: Client, message):
   ...
   if get.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR]:
      ...
      return await message.reply_text("**♪ بە سەرکەوتوویی فەرمانی ئایدی کرایەوە 💎.**")
   else:
      return await message.reply_text("**♪ ببورە ئەزیزم ئەم فەرمانە بۆ ئەدمینەکانە 💎.**")
...
async def muid(client: Client, message):
       if message.chat.id in iddof:
         return await message.reply_text("**♪ فەرمانی ئایدی داخراوە لەلایەن ئەدمینەکان 💎.**")
       ...
       if not id.get(message.from_user.id):
         id[user.id] = []
       idd = len(id[user.id])
       await message.reply_photo(photo=photo, caption=f"**● ꒐ نـاوت : {mentoin}\n● ꒐ ئـایدی : `{user_id}`\n● ꒐ یـوزەرت : [@{username}]\n● ꒐ بـایـۆ : {bioo}**",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{idd} 🤍", callback_data=f"heart{user_id}")],]),)
