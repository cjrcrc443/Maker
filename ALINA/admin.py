...
from ALINA.Data import get_call, get_channel, get_dev, get_group
...
from config import OWNER, VIDEO
    ...
    try:
            ...
            if not CallbackQuery.from_user.id == dev:
                if not CallbackQuery.from_user.username in OWNER:
                    return await CallbackQuery.answer(
                        "پێویستە ئەدمین بیت بۆ کردنی ئەم کارە !", show_alert=True
                    )
        command = CallbackQuery.matches[0].group(1)
        chat_id = CallbackQuery.message.chat.id
        if not await is_served_call(client, chat_id):
            return await CallbackQuery.answer(
                "هیچ پەخشکردنێك بوونی نییە لە ئێستادا .", show_alert=True
            )
        ...
        if command == "pause":
            ...
            await CallbackQuery.answer(
                "پەخشکردن وەستا بۆ ماوەیەکی کاتی ☕🍀", show_alert=True
            )
            await CallbackQuery.message.reply_text(
                f"{CallbackQuery.from_user.mention} **پەخشکردن وەستا بۆماوەیەکی کاتی لەلایەن**"
            )
        if command == "resume":
            await call.resume_stream(chat_id)
            await CallbackQuery.answer("پەخشکردن دەستی پێکردەوە ☕🍀", show_alert=True)
            await CallbackQuery.message.reply_text(
                f"{CallbackQuery.from_user.mention} **پەخشکردن دەستی پێکردەوە لەلایەن**"
            )
        if command == "stop":
            try:
            ...
            await CallbackQuery.answer(
                "بە سەرکەوتوویی پەخشکردن وەستا|ڕاگرترا ⚡", show_alert=True
            )
            await CallbackQuery.message.reply_text(
                f"{CallbackQuery.from_user.mention} **پەخشکردن وەستا|ڕاگرترا لەلایەن**"
            )
    except:
        pass
    ...
    try:
                ...
                if not message.from_user.id == dev:
                    if not message.from_user.username in OWNER:
                        return await message.reply_text(
                            "**پێویستە ئەدمین بیت بۆ کردنی ئەم کارە !**"
                        )
        command = message.command[0]
        chat_id = message.chat.id
        if not await is_served_call(client, chat_id):
            return await message.reply_text(
                "**هیچ پەخشکردنێك بوونی نییە لە ئێستادا .**"
            )
        call = await get_call(bot_username)
        chat_id = message.chat.id
        if command == "/pause" or command == "وەستانی کاتی":
            ...
            if not check:
                ...
                await remove_active(bot_username, chat_id)
                return await message.reply_text(
                    "**پەخشکردنم وەستاند چونکە لیستی گۆرانی بەتاڵە 🎻.**"
                )
            ...
            if file:
                ...
                try:
                    ...
                    return client.send_message(
                        chat_id, "**هەڵە ڕوویدا لە کاتی پەخشکردنی دواتر 🎻.**"
                    )
            stream = (
                AudioVideoPiped(
                    file_path,
