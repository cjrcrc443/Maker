...
from config import MONGO_DB_URL, OWNER, botname, call, dev, logger, logger_mode, user
...
async def set_bot_name(bot_username: dict, BOT_NAME: str):
    ...
    bot_name.update_one(
        {"bot_username": bot_username}, {"$set": {"bot_name": BOT_NAME}}, upsert=True
    )
...
async def set_group(bot_username: dict, group: str):
    ...
    groupdb.update_one(
        {"bot_username": bot_username}, {"$set": {"group": group}}, upsert=True
    )
...
async def set_channel(bot_username: dict, channel: str):
    ...
    channeldb.update_one(
        {"bot_username": bot_username}, {"$set": {"channel": channel}}, upsert=True
    )
...
async def set_video_source(bot_username: dict, VID_SO: str):
    ...
    vid_so.update_one(
        {"bot_username": bot_username}, {"$set": {"vid_so": VID_SO}}, upsert=True
    )
...
async def set_dev_user(bot_username: dict, DEV_USERR: str):
    ...
    dev_userr.update_one(
        {"bot_username": bot_username}, {"$set": {"dev_userr": DEV_USERR}}, upsert=True
    )
...
async def set_groupsr(bot_username: dict, groupsr: str):
    ...
    groupdbsr.update_one(
        {"bot_username": bot_username}, {"$set": {"groupsr": groupsr}}, upsert=True
    )
...
async def set_channelsr(bot_username: dict, channelsr: str):
    ...
    channeldbsr.update_one(
        {"bot_username": bot_username}, {"$set": {"channelsr": channelsr}}, upsert=True
    )
...
@Client.on_message(
    filters.command(["• گۆڕینی کەناڵی سەرچاوە •", "گۆڕینی کەناڵی سەرچاوە"], "")
)
...
@Client.on_message(
    filters.command(["• گۆڕینی گرووپی سەرچاوە •", "گۆڕینی گرووپی سەرچاوە"], "")
)
    ...
    if not userbot:
        ...
        for i in Bots:
            ...
            if bot == bot_username:
                session = i["session"]
                userbot = Client(
                    "ALINA", api_id=API_ID, api_hash=API_HASH, session_string=session
                )
...
async def set_must(bot_username: dict, m: str):
    ...
    mustdb.update_one(
        {"bot_username": bot_username}, {"$set": {"getmust": ii}}, upsert=True
    )
