...
from SEMO.Data import (get_logger, get_userbot, get_call, get_logger_mode, get_group, get_channel, get_dev_name, get_dev)
import asyncio
...
@Client.on_message(filters.command(["عشدژدحوائي", "تشغيلودوودو عشوائي"], ""))
async def aii(client: Client, message):
   ...
   try:
    ...
    chat_id = message.chat.id
    user_id = message.from_user.id if message.from_user else "IQ7amo"
    videoid = None
    link = None
